...
- **Ficheiro de log** (montado em `gNB_open/logs/`):

```bash
tail -f gNB_open/logs/cu.log
```
...
- **NGAP em PCAP** (se ativo no `cu.yml`): `gNB_open/logs/cu_ngap.pcap`.

### DU

```bash
tail -f gNB_open/logs/du.log
