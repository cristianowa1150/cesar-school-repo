...
# Candidatos de RAN
GNB_CONTAINERS=(ueransim-gnb-containerized ueransim srsran-cu-du-containerized srsran-cu-containerized srsran-du-containerized)
...
echo "Open5GS + RAN (UERANSIM / srsRAN CU-DU)"
echo "=========================================="
echo ""
...
for ue in "${UE_CONTAINERS[@]}"; do
    ...
    if ue_has_radio_or_session "$ue"; then
        ...
        echo -e "${YELLOW}⚠️  [$ue] sem evidencia de celula/RRC nos logs atuais${NC}"
    fi
...
if [ "$AMF_CONTEXT_ERROR" -gt 0 ] 2>/dev/null; then
    ...
    echo "Suba uma RAN (./scripts/up_ran.sh) e rode novamente."
    echo ""
